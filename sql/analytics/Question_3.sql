...
WITH customer_totals AS (
    SELECT
        ...
        c.first_name,
        c.last_name,
        SUM(f.line_total) AS total_purchase
...
top_10_customers AS (
    ...
    FROM (
        SELECT *,
               NTILE(10) OVER (ORDER BY total_purchase DESC) AS decile
...
FROM top_10_customers t JOIN
dw.dim_customer c ON t.customer_id = c.customer_id
JOIN dw.fact_retail f ON
